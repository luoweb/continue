# Contributing to AICODER (JetBrains extension) <!-- omit in toc -->
...
AICODER is built with JDK version 17 (as specified in [`./build.gradle.kts`](./build.gradle.kts)), which can be
...
Select the `Run AICODER` task in the top right corner of the IDE and then select the "Debug" option.

> In community edition, use `Run AICODER (CE)` instead, which uses shell scripts instead of Ultimate-only node configs.
...
When running the `Start Core Dev Server` task, we set the location of your AICODER directory to
...
When using the `Run AICODER` task, we automatically tail both prompt logs and IDE logs.
