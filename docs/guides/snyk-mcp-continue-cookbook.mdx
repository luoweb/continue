...
<Steps>
  ...
  <Step title="Connect Snyk MCP via Continue Hub">
    Visit the [Snyk MCP on Continue Hub](https://hub.roweb.cn/snyk/snyk-mcp) and click **Install** to add it to the agent you created in the step above.
  ...
  <Step title="Add Secure-at-Inception Rules">
    Install the [Snyk Secure-at-Inception rules](https://hub.roweb.cn/snyk/secure-at-inception) from the Hub to enable automatic security scanning.
...
<Check>
  **Verification Steps:** - Snyk MCP is installed via [Continue
  Hub](https://hub.roweb.cn/snyk/snyk-mcp) - Secure-at-Inception rules are
  [enabled](https://hub.roweb.cn/snyk/secure-at-inception) - Authentication
