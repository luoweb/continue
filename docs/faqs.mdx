...
For centralized team secret management, use `${{ inputs.SECRET_NAME }}` syntax in your config.yaml and manage them at [https://hub.roweb.cn/settings/secrets](https://hub.roweb.cn/settings/secrets):
```yaml
models:
  - provider: openai
...
You can leverage model addons from the Continue Hub in your local configurations using the `uses:` syntax. This allows you to reference pre-configured model blocks without duplicating configuration.
